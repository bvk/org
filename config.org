...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/apt.conf.d/99force-ipv4" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/sources.list.d/stable.list" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/preferences.d/stable.pref" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/sources.list.d/testing.list" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/preferences.d/testing.pref" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/sources.list.d/unstable.list" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/preferences.d/unstable.pref" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/sources.list.d/experimental.list" "no")
...
#+BEGIN_SRC conf :tangle (if (eq system-type 'gnu/linux) "/sudo::/etc/apt/preferences.d/experimental.pref" "no")
# 0 < P < 100: causes a version to be installed only if there is no
# installed version of the package
...
The following ~vterm_printf~ function enables SHELL integration with the Emacs
vterm.
...
(defun display-buffer-in-leftmost-window (buffer alist)
  ...
  (let ((leftmost-window (left-most-window)))
    ...
    ;; Decrease the font size by 2 steps.
    (with-current-buffer buffer (text-scale-decrease 2))
    leftmost-window))
...
#+BEGIN_SRC emacs-lisp :mkdirp yes :tangle ~/.emacs.d/my-ui.el
(use-package nerd-icons
  :custom
  ;; The Nerd Font you want to use in GUI
  ;; "Symbols Nerd Font Mono" is the default and is recommended
  ;; but you can use any other Nerd Font if you want
  (nerd-icons-font-family "Ubuntu Mono Nerd Font")
  )
#+END_SRC
...
;; FIXME: These paths don't make sense for Mac OS X.
...
#+BEGIN_SRC emacs-lisp :mkdirp yes :tangle ~/.emacs.d/my-llm.el
(if (file-exists-p "~/code/extern/whisper.el")
    (use-package whisper
      :load-path "~/code/extern/whisper.el"
      :config
      (setq whisper-install-directory "~/opt/"
            whisper-model "base"
            whisper-language "en"
...
#+BEGIN_SRC text :tangle (if (eq system-type 'gnu/linux) "~/.Xresources" "no")
selectToClipboard: true
#+END_SRC

#+BEGIN_SRC text :tangle (if (eq system-type 'gnu/linux) "~/.Xresources" "no")
xterm*faceName: xft:UbuntuSansMono
#+END_SRC
...
xterm*VT100*translations: #override \n\
  ...
  Ctrl Shift <Key> S: smaller-vt-font()
#+END_SRC
...
#+BEGIN_SRC conf :mkdirp yes :tangle (if (eq system-type 'gnu/linux) "~/.config/sxhkd/sxhkdrc" "no")
...
super + t
	xterm -rv -fa xft:'UbuntuSansMono-14'
...
#+BEGIN_SRC bash :tangle (if (eq system-type 'gnu/linux) "~/.xsession" "no")
#!/bin/bash
source $HOME/.bashrc
#+END_SRC
...
#+BEGIN_SRC bash :tangle (if (eq system-type 'gnu/linux) "~/.xsession-steam" "no")
...
#+BEGIN_SRC bash :tangle (if (eq system-type 'gnu/linux) "~/.xsession-gold" "no")
